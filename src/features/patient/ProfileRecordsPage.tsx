...
export function ProfileRecordsPage(_props: ProfileRecordsPageProps = {}) {
  ...
  const [activeTab, setActiveTab] = useState<
    "personal" | "testHistory" | "medical" | "files"
  >(getInitialTab());
  ...
  const [showUpdateSuccessNotification, setShowUpdateSuccessNotification] =
    useState(false);
  const [showUpdateErrorNotification, setShowUpdateErrorNotification] =
    useState(false);
  const [showAvatarSuccessNotification, setShowAvatarSuccessNotification] =
    useState(false);
  const [showAvatarErrorNotification, setShowAvatarErrorNotification] =
    useState(false);
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-white/80 text-sm">
                Theo dõi các chỉ số quan trọng của bạn
              </p>
            </div>
            <button
              onClick={() => setShowTestModal(true)}
              className="bg-white/20 hover:bg-white/30 px-4 py-2 rounded-xl font-medium transition-colors text-sm flex items-center gap-2"
            >
              <Plus size={16} />
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200">
                <th className="text-left py-4 px-4 font-semibold text-[#0F172A]">
                  Ngày
                </th>
                <th className="text-left py-4 px-4 font-semibold text-[#0F172A]">
            ...
            <tbody>
              {panelsLoading && (
                <tr>
                  <td className="py-6 px-4 text-gray-500" colSpan={10}>
                    Đang tải dữ liệu...
                  </td>
                </tr>
              )}
              ...
              {!panelsLoading &&
                testResults.map((result, index) => {
                  ...
                  return (
                    ...
                    >
                      <td className="py-4 px-4 text-[#0F172A] font-medium">
                        {result.date}
                      </td>
                      <td className="py-4 px-4 text-[#0F172A]">
                        {fmt("serum_creatinine", " mg/dL", result.creatinine)}
                      </td>
                      <td className="py-4 px-4 text-[#0F172A]">
                        {fmt("gfr", " ml/min", result.egfr)}
                      </td>
                      <td className="py-4 px-4 text-[#0F172A]">
                        {fmt("bun", " mg/dL", result.bun)}
                      </td>
                      <td className="py-4 px-4 text-[#0F172A]">
                        {fmt("serum_calcium", " mg/dL")}
                      </td>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-blue-700">
                Các kết quả xét nghiệm được cập nhật định kỳ để theo dõi tình
                trạng sức khỏe của bạn. Nếu có bất kỳ thắc mắc nào về kết quả,
  ...
  const renderPersonalInfo = () => (
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              {isEditing && (
                <label
                  className={`absolute -bottom-1 -right-1 w-6 h-6 bg-white rounded-full flex items-center justify-center shadow-lg hover:shadow-xl transition-shadow ${isUploadingAvatar ? "cursor-not-allowed opacity-50" : "cursor-pointer"}`}
                >
          ...
          <div className="flex gap-3">
            {!isEditing ? (
              <button
                ...
                className="bg-white/20 hover:bg-white/30 px-4 py-2 rounded-xl font-medium transition-colors text-sm"
              >
                <span>Chỉnh sửa</span>
              </button>
            ) : (
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="bg-white text-[#1E75FF] hover:bg-gray-100 px-4 py-2 rounded-xl font-medium transition-colors flex items-center gap-2 disabled:opacity-50 text-sm"
                >
                  {isSaving || isUpdating ? (
                    <div className="w-4 h-4 border-2 border-[#1E75FF] border-t-transparent rounded-full animate-spin" />
                  ) : (
                    <Save size={16} />
                  )}
                  <span>
                    {isSaving || isUpdating ? "Đang lưu..." : "Lưu thay đổi"}
                  </span>
                </button>
              </div>
            )}
      ...
      <div
        className="bg-white rounded-2xl shadow-[0_10px_24px_rgba(16,24,40,0.08)] p-8"
        style={{
          display: "none",
        }}
      >
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-[0_10px_24px_rgba(16,24,40,0.08)] p-8">
          ...
          {!medicalRecordsLoading &&
            !medicalRecordsError &&
            medicalRecords.length > 0 && (
              <MedicalRecordTimeline allRecords={medicalRecords as any} />
            )}
          ...
          <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-xl">
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-blue-700">
                  Hồ sơ khám bệnh được lưu trữ an toàn và chỉ bạn cùng bác sĩ
                  điều trị có quyền truy cập. Vui lòng tuân thủ đơn thuốc và
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
        ...
        <div className="space-y-3">
          {medicalFiles.map((file) => (
            <div
              key={file.id}
              className={`flex items-center space-x-4 p-4 border rounded-xl transition-colors ${isSelectionMode && selectedFiles.includes(file.id) ? "border-blue-500 bg-blue-50" : "border-gray-200 hover:bg-gray-50"}`}
            >
              ...
              <div className="flex items-center space-x-2">
                {isSelectionMode && (
                  <button
                    ...
                    className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${selectedFiles.includes(file.id) ? "bg-blue-600 border-blue-600 text-white" : "border-gray-300 hover:border-blue-400"}`}
                  >
  ...
  const handleAddTestResult = async () => {
    ...
    if (ok) {
      ...
      toast.error("Thêm kết quả xét nghiệm thất bại", {
        description: createPanelError || "Vui lòng thử lại sau",
        duration: 3000,
      });
    }
      ...
      <AnimatePresence>
        {showAvatarErrorNotification && (
          ...
          >
            ...
            <span>
              {avatarError ||
                "Có lỗi xảy ra khi upload ảnh đại diện. Vui lòng thử lại!"}
            </span>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {showTestModal && (
          <motion.div
            ...
            onClick={(e) =>
              e.target === e.currentTarget && handleCancelAddTest()
            }
          >
            <motion.div
              initial={{
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleNewTestInputChange("creatinine", e.target.value)
                        }
                        className="w-full px-4 py-3 border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#1E75FF] focus:border-transparent transition-all"
                        placeholder="1.0"
                        required
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          handleNewTestInputChange("hematuria", e.target.value)
                        }
                        className="w-full px-4 py-3 border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#1E75FF] focus:border-transparent transition-all"
                      >
                        <option value="">Chọn kết quả</option>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleNewTestInputChange("urinePH", e.target.value)
                        }
                        className="w-full px-4 py-3 border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#1E75FF] focus:border-transparent transition-all"
                        placeholder="7.0"
                      />
