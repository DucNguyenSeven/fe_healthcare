...
import { Send, Bot, User, Upload, FileText, Clock, Trash2, MessageCircle, AlertTriangle, Activity, Droplets, Heart, Calculator, TrendingUp, Shield, AlertCircle, ChevronRight, ChevronLeft, BarChart3, Calendar } from 'lucide-react';
import { 
  User as UserType, 
  ChatMessage, 
  ChatHistory, 
...
Bạn có thể đặt thêm câu hỏi cụ thể hoặc sử dụng các gợi ý bên dưới để tôi có thể hỗ trợ tốt hơn.`;
  ...
  const handleBookAppointment = () => {
    ...
    // Đánh dấu cần mở form đặt lịch
    localStorage.setItem('open_booking_form', 'true');
  ...
  const renderUpload = () => <div className="max-w-4xl mx-auto space-y-6">
      {/* Upload Area */}
  ...
  const renderCKDPrediction = () => <div className="max-w-6xl mx-auto space-y-6">
  ...
  return <div className="h-full flex flex-col">
      ...
      <div className="bg-white border-b border-gray-200 px-4 lg:px-6">
        <div className="flex space-x-4 lg:space-x-8 overflow-x-auto">
          <button onClick={() => setCurrentView('chat')} className={`py-4 px-2 border-b-2 font-medium text-sm whitespace-nowrap min-w-fit ${currentView === 'chat' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
            Trò chuyện
          </button>
          <button onClick={() => setCurrentView('ckd-prediction')} className={`py-4 px-2 border-b-2 font-medium text-sm whitespace-nowrap min-w-fit ${currentView === 'ckd-prediction' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
            Dự đoán CKD
          </button>
          <button onClick={() => setCurrentView('upload')} className={`py-4 px-2 border-b-2 font-medium text-sm whitespace-nowrap min-w-fit ${currentView === 'upload' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
            Phân tích file
          </button>
          <button onClick={() => setCurrentView('history')} className={`py-4 px-2 border-b-2 font-medium text-sm whitespace-nowrap min-w-fit ${currentView === 'history' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
            Lịch sử
          </button>
        </div>
      </div>
      ...
      {/* Content */}
      <div className={`flex-1 ${currentView === 'chat' ? '' : 'p-4 lg:p-6'} ${currentView === 'ckd-prediction' || currentView === 'upload' ? 'overflow-y-auto' : 'overflow-hidden'}`}>
