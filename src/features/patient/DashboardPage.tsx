...
import { Plus, Calendar, TrendingUp, TrendingDown, AlertTriangle, Clock, MessageCircle, FileText, Activity, Heart, Droplets, Weight, CheckCircle2, ArrowRight, BookOpen, Video } from 'lucide-react';
import { User, Appointment, HealthMetric, Alert, NavigationItem } from './HealthcarePlusApp';
...
export function DashboardPage({
  ...
  const criticalAlerts = alerts.filter(alert => !alert.isRead && alert.type === 'critical');
  ...
  return <div className="p-4 lg:p-6 space-y-6">
      {/* Welcome Section */}
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 rounded-2xl p-6 text-white" style={{
      background: "linear-gradient(90deg, oklch(0.546 0.245 262.881) 0%, oklch(0.488 0.243 264.376) 100%)"
        ...
        <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getCKDStageColor(user.ckdStage)}`} style={{
        display: "none"
      }}>
      ...
      {/* Critical Alerts */}
      {criticalAlerts.length > 0 && <div className="bg-red-50 border border-red-200 rounded-2xl p-4" style={{
      display: "none"
    }}>
          <div className="flex items-start space-x-3">
            <AlertTriangle className="w-6 h-6 text-red-600 flex-shrink-0 mt-0.5" />
            <div className="flex-1">
              <h3 className="text-lg font-semibold text-red-900 mb-2">Cảnh báo quan trọng</h3>
              {criticalAlerts.map(alert => <div key={alert.id} className="mb-3 last:mb-0">
      ...
      <div>
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          return <button key={action.id} onClick={action.onClick} className="flex flex-col items-center justify-center p-6 bg-white rounded-2xl border border-gray-200 hover:shadow-lg transition-all duration-200 hover:scale-105">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => onNavigate('monitoring')} className="text-blue-600 hover:text-blue-700 text-sm font-medium flex items-center">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              return <div key={metric.id} className="p-4 bg-gray-50 rounded-xl">
            ...
            <div className="space-y-3">
              <div className="flex items-center space-x-3 p-3 bg-gray-50 rounded-xl">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600">Tư vấn thận học - 12/01/2024</p>
                </div>
              ...
              <div className="flex items-center space-x-3 p-3 bg-gray-50 rounded-xl">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600">Khám tổng quát - 08/01/2024</p>
                </div>
                <button className="text-blue-600 hover:text-blue-700 text-sm font-medium">
                  Xem lại
                </button>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            ...
            {upcomingAppointments.length > 0 ? <div className="space-y-3">
                {upcomingAppointments.map(appointment => <div key={appointment.id} className="p-3 bg-blue-50 rounded-xl">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-medium text-blue-900">{appointment.time}</span>
                      {appointment.canJoin && <span className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded-full">
                          Có thể vào
                        </span>}
                    </div>
                ...
                <button onClick={() => onNavigate('appointments')} className="w-full mt-3 text-blue-600 hover:text-blue-700 text-sm font-medium flex items-center justify-center">
                  Xem tất cả <ArrowRight className="w-4 h-4 ml-1" />
                </button>
              </div> : <div className="text-center py-4">
                <Clock className="w-8 h-8 text-gray-400 mx-auto mb-2" />
                <p className="text-gray-500 text-sm">Không có lịch hẹn hôm nay</p>
                <button onClick={() => onNavigate('appointments')} className="mt-2 text-blue-600 hover:text-blue-700 text-sm font-medium">
                  Đặt lịch mới
                </button>
              </div>}
          </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={() => onNavigate('community')} className="text-blue-600 hover:text-blue-700 text-sm font-medium flex items-center">
            Xem thêm <ArrowRight className="w-4 h-4 ml-1" />
          </button>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {mockArticles.map(article => <div key={article.id} className="group cursor-pointer">
