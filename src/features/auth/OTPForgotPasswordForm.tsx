...
import { useOTPForgotPassword } from '@/hooks/useOTPForgotPassword';
import { toast } from 'sonner';

interface OTPForgotPasswordFormProps {
  onNavigate: (page: 'login' | 'register' | 'forgot-password' | 'reset-password', email?: string, otp?: string) => void;
  userEmail: string;
}

export const OTPForgotPasswordForm = ({
  onNavigate,
  userEmail
}: OTPForgotPasswordFormProps) => {
  const [otp, setOtp] = useState(['', '', '', '', '', '']);
  const [focusedIndex, setFocusedIndex] = useState(0);
  const inputRefs = useRef<(HTMLInputElement | null)[]>([]);
  
  // Use the custom hook for OTP forgot password logic
  const { 
    isLoading, 
    error, 
    resendCooldown, 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const success = await validateOTP(userEmail, otpValue);
    
    if (success) {
      // Show success notification
      toast.success('Xác thực OTP thành công!', {
        description: 'Mã OTP hợp lệ. Bạn có thể tạo mật khẩu mới',
        duration: 3000,
      });
      
      // Navigate to reset password page after successful OTP validation
      setTimeout(() => {
        onNavigate('reset-password', userEmail, otpValue);
    ...
    >
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-gray-600 text-sm leading-relaxed mb-2">
          <span>
            Chúng tôi đã gửi mã OTP để đặt lại mật khẩu tới email của bạn, vui lòng nhập mã để tiếp tục.
          </span>
        </p>
                ...
                className={`
                  ...
                  ${error 
                    ...
                    : focusedIndex === index || digit 
                      ...
                      : 'border-gray-200 bg-gray-50/50 text-gray-900 hover:border-gray-300 hover:bg-white'
                  }
                `}
                whileFocus={{ scale: 1.05 }}
                whileHover={{ scale: digit ? 1 : 1.02 }}
              />
            ))}
          </div>
