...
export const DoctorProfilePage = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const userFieldsToUpdate = Object.keys(updateData).filter(
        (key) => key !== "userId" && key !== "role"
      );
      const doctorFieldsToUpdate = Object.keys(doctorUpdateData).filter(
        (key) => key !== "userId"
      );

      if (
        userFieldsToUpdate.length === 0 &&
        doctorFieldsToUpdate.length === 0
      ) {
  ...
  const handleAddCertificate = async () => {
    if (!doctor?.userId) {
      toast.error("Thêm chứng chỉ thất bại", {
        description: "Không tìm thấy thông tin bác sĩ. Vui lòng thử lại.",
        duration: 3000,
      });
      return;
    }
    ...
    if (
      ...
      newCertificate.issuingOrganization.trim()
    ) {
      ...
      // Sử dụng callback trong mutate options vì addCertification là mutate function
      addCertification(certData, {
        onSuccess: async () => {
          // Invalidate và refetch cả doctor data và certifications để lấy dữ liệu mới nhất
          await Promise.all([
            queryClient.invalidateQueries({ queryKey: ["doctor", "byId"] }),
          ...
          toast.success("Thêm chứng chỉ thành công!", {
            description: "Chứng chỉ đã được thêm vào hồ sơ của bạn",
            duration: 3000,
          });
        },
        onError: (error: any) => {
          const errorMessage =
            error?.message ||
            "Có lỗi xảy ra khi thêm chứng chỉ. Vui lòng thử lại.";
          toast.error("Thêm chứng chỉ thất bại", {
  ...
  const handleUpdateCertificate = async () => {
    if (!doctor?.userId || !editingCertificate) {
      toast.error("Cập nhật chứng chỉ thất bại", {
        description:
          "Không tìm thấy thông tin bác sĩ hoặc chứng chỉ. Vui lòng thử lại.",
        duration: 3000,
      });
      return;
    }
    ...
    if (
      ...
      editCertificate.issuingOrganization.trim()
    ) {
      ...
      // Sử dụng callback trong mutate options vì updateCertification là mutate function
      updateCertification(
        {
          certificationId: editingCertificate.id,
          certData,
        },
        {
          onSuccess: async () => {
            ...
            toast.success("Cập nhật chứng chỉ thành công!", {
              description: "Thông tin chứng chỉ đã được cập nhật",
              duration: 3000,
            });
          },
          onError: (error: any) => {
            const errorMessage =
              error?.message ||
              "Có lỗi xảy ra khi cập nhật chứng chỉ. Vui lòng thử lại.";
            toast.error("Cập nhật chứng chỉ thất bại", {
              description: errorMessage,
              duration: 3000,
            });
  ...
  const handleDeleteCertificate = async (id: string) => {
    ...
    // Sử dụng callback trong mutate options vì deleteCertification là mutate function
    deleteCertification(id, {
      onSuccess: async () => {
        ...
        toast.success("Xóa chứng chỉ thành công!", {
          description: "Chứng chỉ đã được xóa khỏi hồ sơ của bạn",
          duration: 3000,
        });
      },
      onError: (error: any) => {
        ...
        const errorMessage =
          error?.message ||
          "Có lỗi xảy ra khi xóa chứng chỉ. Vui lòng thử lại.";
        toast.error("Xóa chứng chỉ thất bại", {
          description: errorMessage,
          duration: 3000,
        });
            ...
            <div className="flex gap-3">
              {!isEditing ? (
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={handleSave}
                    disabled={isSaving || isUpdating || isUpdatingDoctor}
                  ...
                  >
                    {isSaving || isUpdating || isUpdatingDoctor ? (
              ...
              {isEditing ? (
                ...
                <div className="px-4 py-3 bg-gray-50 rounded-2xl">
                  <p className="text-[#0F172A] leading-relaxed">
                    {formData.introduction || "Chưa có giới thiệu"}
                  </p>
                </div>
              )}
            </div>
              ...
              <div className="space-y-2">
                ...
                {isEditing ? (
                  ...
                  <div className="flex items-center gap-3 px-4 py-3 bg-gray-50 rounded-2xl">
                    ...
                    <span className="text-[#0F172A]">
                      {doctorData.specialty || "Chưa cập nhật"}
                    </span>
                  </div>
                )}
              </div>
                  ...
                  <div className="flex items-center gap-3 px-4 py-3 bg-gray-50 rounded-2xl">
                    ...
                    <span className="text-[#0F172A]">
                      {doctorData.experienceYears
                        ? `${doctorData.experienceYears} năm`
                        : "Chưa cập nhật"}
                    </span>
                  </div>
                )}
                ...
                {isEditing ? (
                  <input
                    ...
                    onChange={(e) =>
                      setDoctorData((prev) => ({
                        ...prev,
                        examinationFee: parseInt(e.target.value) || 0,
                      }))
                    }
                    className="w-full px-4 py-3 border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#1E75FF] focus:border-transparent transition-all"
                    placeholder="500000"
                    ...
                    <span className="text-[#0F172A]">
                      {doctorData.examinationFee
                        ? new Intl.NumberFormat("vi-VN", {
                            style: "currency",
                            currency: "VND",
                          }).format(doctorData.examinationFee)
                        : "Chưa cập nhật"}
                    </span>
                  </div>
                )}
              ...
              <div className="space-y-2">
                ...
                {isEditing ? (
                  ...
                  <div className="flex items-center gap-3 px-4 py-3 bg-gray-50 rounded-2xl">
                    <MapPin size={20} className="text-[#334155]" />
                    <span className="text-[#0F172A]">
                      {doctorData.clinicAddress || "Chưa cập nhật"}
                    </span>
                  </div>
                )}
              </div>
                  ...
                  >
                    <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      >
                        ...
                        showDeleteConfirm === certificate.id ? (
                          <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
                        ) : (
                          <X size={16} />
                        )}
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-[#0F172A] mb-2 pr-20">
                          {certificate.name}
                        </h3>
              ...
              >
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <select
                      value={newCertificate.yearIssued}
                      onChange={(e) =>
                        setNewCertificate((prev) => ({
                          ...prev,
