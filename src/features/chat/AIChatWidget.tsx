...
export function AIChatWidget() {
  ...
  const messages = useMemo(() => {
    if (!currentAIGroupId || !allMessages[currentAIGroupId] || allMessages[currentAIGroupId].length === 0) {
      return [{
        id: 'welcome',
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 flex flex-col-reverse gap-2">
              ...
              {messages.map((message, index) => {
                ...
                return (
                  <div key={message.id || `msg-${index}`} className={`flex ${isOwn ? 'justify-end' : 'justify-start'} mb-4`}>
                    <div className={`flex ${isOwn ? 'flex-row-reverse' : 'flex-row'} items-end space-x-2 max-w-[85%]`}>
                      {!isOwn && (
